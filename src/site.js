...
const ui = {
  ...
  farmerEyebrow: b("FARMER-FIRST THINKING", "రైతు-మొదటి ఆలోచన"), farmerTitle: b("Built Around Real Crop Needs", "రైతుల పంట అవసరాలను దృష్టిలో పెట్టుకుని రూపొందించిన పరిష్కారాలు"), farmerCopy: b("A practical way to navigate the crop stages and needs that shape our portfolio.", "మా ఉత్పత్తుల శ్రేణిని రూపొందించే పంట దశలు, అవసరాలను అర్థం చేసుకోవడానికి ఆచరణాత్మక మార్గం."), cropFieldScene: b("Farmers working in a cotton field", "పత్తి పొలంలో పనిచేస్తున్న రైతులు"),
...
function productCard(product) {
  return `<article class="product-card product-card--${product.id}">
    <div class="product-card__visual product-card__visual--${product.id}"><span class="product-card__category">${pick(categoryLabels[product.category])}</span><div class="product-card__halo"></div><img src="${product.image}" alt="${product.name} agricultural product package" loading="lazy"></div>
...
function productModal() {
  ...
  return `<div class="modal-backdrop" data-modal-backdrop role="presentation">
    <section class="product-modal" role="dialog" aria-modal="true" aria-labelledby="product-modal-title">
      ...
      <div class="product-modal__visual product-modal__visual--${product.id}"><div class="product-modal__ring"></div><img src="${product.image}" alt="${product.name} product pack"><span>${pick(categoryLabels[product.category])}</span></div>
      <div class="product-modal__content"><p class="eyebrow">${pick(categoryLabels[product.category])}</p><h2 id="product-modal-title">${product.name}</h2><p class="modal-intro">${pick(product.overview)}</p>
        ...
        <div class="modal-actions"><button class="button button--green" type="button" data-enquire-product="${product.id}">${pick(ui.enquiry)}${arrow()}</button><a class="button button--outline" href="tel:+919133243325">${pick(ui.call)}</a><a class="modal-whatsapp" href="https://wa.me/917981312887?text=${encodeURIComponent(`Hello Stemgrow, I would like to enquire about ${product.name}.`)}" target="_blank" rel="noreferrer">${whatsappIcon()}<span>WhatsApp</span></a><button class="modal-back" type="button" data-close-modal>${pick(ui.backProducts)}${arrow()}</button></div><p class="label-note">${pick(ui.policy)}</p>
      </div>
    </section>
  </div>`;
}
  ...
  root.innerHTML = replaceDecorativeSymbols(`
    ...
    <main>
      <section class="hero" id="home"><div class="hero__media" role="img" aria-label="${pick(ui.heroScene)}"><span class="hero__slide hero__slide--cotton is-active" aria-hidden="true"></span><span class="hero__slide hero__slide--maize" aria-hidden="true"></span><span class="hero__slide hero__slide--mirchi" aria-hidden="true"></span></div><div class="hero__overlay"></div>
        ...
        </div><div class="hero__crop-labels" aria-hidden="true"><span>${pick(ui.heroCotton)}</span><span>${pick(ui.heroMaize)}</span></div><div class="hero__leaf" aria-hidden="true"></div>
      ...
      <section class="section approach" id="approach"><div class="approach__field" aria-hidden="true"></div><div class="container">${heading(ui.approachEyebrow, ui.approachTitle, ui.approachCopy, true)}<div class="approach-grid">${approach.map(([number, icon, title, copy]) => `<article class="approach-card reveal" data-index="${number}"><span class="approach-card__watermark" aria-hidden="true">${number}</span><div class="approach-card__top"><span class="approach-card__icon">${icon}</span><span>${number}</span></div><h3>${pick(title)}</h3><p>${pick(copy)}</p><span class="approach-card__rule" aria-hidden="true"></span></article>`).join("")}</div></div></section>
    ...
    <a class="whatsapp-float" href="https://wa.me/917981312887" target="_blank" rel="noreferrer" aria-label="${locale === "en" ? "Chat with Stemgrow on WhatsApp" : "వాట్సాప్‌లో స్టెమ్‌గ్రోతో మాట్లాడండి"}">${whatsappIcon()}<span>WhatsApp</span></a>
    ${productModal()}`);
...
function startHeroSlideshow() {
  if (heroSlideTimer !== null && typeof window.clearInterval === "function") window.clearInterval(heroSlideTimer);
  heroSlideTimer = null;
